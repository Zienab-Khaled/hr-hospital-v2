...
@section('tabs')
    <a href="{{ route('dashboard') }}"
        class="px-4 py-2 rounded-t bg-blue-600  text-sm font-medium">{{ __('Patient File') }}</a>
    @can('invoices.view')
        <a href="{{ route('invoices.index') }}"
            class="px-4 py-2 rounded-t bg-slate-100 text-slate-600 text-sm hover:bg-slate-200">{{ __('Invoices') }}</a>
    @endcan
    @can('authorizations.view')
        <a href="{{ route('authorizations.index') }}"
            class="px-4 py-2 rounded-t bg-slate-100 text-slate-600 text-sm hover:bg-slate-200">{{ __('Authorizations') }}</a>
    @endcan

    {{-- Quick Action Tabs (حسب الصلاحيات — توديك لصفحات الإنشاء) --}}
    @can('visits.create')
        <a href="{{ route('visits.create') }}"
            class="ml-4 px-4 py-2 rounded-t bg-green-600 text-sm font-bold hover:bg-green-700 flex items-center gap-1 "
    ...
    @if (\App\Support\RoleNav::canCreateInvoiceWithServices(auth()->user()))
        <a href="{{ route('invoices.create') }}"
            class="ml-1 px-4 py-2 rounded-t bg-amber-600 text-sm font-bold hover:bg-orange-600 flex items-center gap-1 ">
...
@section('content')
    ...
    <div id="dashboardModal" class="hidden"
        style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(15, 23, 42, 0.5); z-index: 99999; padding: 1rem; align-items: center; justify-content: center; display: none;">
            ...
            <div class="p-6">
                <form id="dashboardModalForm" onsubmit="handleDashboardSubmit(event)">
                    ...
                    <div id="patientSearchSection" class="mb-4">
                        ...
                        <div class="relative">
                            <input type="text" id="patientSearchInput" oninput="debounceSearch(this.value)"
                                autocomplete="off"
                            ...
                            <div id="patientSearchResults"
                                class="absolute z-10 w-full bg-white shadow-lg rounded-b-lg border border-slate-200 mt-1 hidden max-h-48 overflow-y-auto">
                            </div>
                        </div>
                        <input type="hidden" id="selectedPatientId" name="patient_id" required>
                        <div id="selectedPatientBadge"
                            class="mt-2 hidden px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium flex justify-between items-center">
                    ...
                    <div id="invoiceSearchSection" class="mb-4 hidden">
                        ...
                        <select id="invoiceSelect" name="invoice_id"
                            class="w-full rounded-lg border-2 border-slate-300 px-4 py-2 focus:border-blue-500 focus:outline-none">
                            <option value="">
                                {{ app()->getLocale() === 'ar' ? 'اختر الفاتورة...' : 'Select invoice...' }}</option>
                        </select>
                    </div>
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        <button type="button" onclick="closeDashboardModal()"
                            class="px-6 py-2 rounded-lg border border-slate-300 text-slate-600 font-semibold hover:bg-slate-50">
                            {{ app()->getLocale() === 'ar' ? 'إلغاء' : 'Cancel' }}
                        </button>
                        <button type="submit" id="submitBtn"
                            class="px-8 py-2 bg-blue-600  rounded-lg font-bold hover:bg-blue-700 disabled:opacity-50">
        ...
        @can('visits.view')
            <div class="bg-white rounded-lg shadow p-6 border border-slate-200">
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    @can('visits.create')
                        <a href="{{ route('visits.create') }}"
                            class="bg-green-600  px-4 py-2 rounded text-white text-sm font-bold hover:bg-green-700">+
                            {{ __('Add Visit') }}</a>
                    @endcan
                    <a href="{{ route('visits.index') }}"
                        class="bg-slate-100 text-slate-600 px-4 py-2 rounded text-sm hover:bg-slate-200">{{ __('Full Page') }}</a>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-slate-600">
                        <thead>
                            <tr class="border-b">
                                <th class="text-start py-2">{{ __('Patients') }}</th>
                                <th class="text-start py-2">{{ __('Department') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($recentVisits as $v)
                                <tr class="border-b border-slate-100 hover:bg-slate-50 cursor-pointer"
                                    onclick="openDashboardModal('view', {type: 'visit', id: {{ $v->id }}})">
                                    <td class="py-2 text-blue-600 font-medium">{{ $v->patient?->name }}</td>
                                    <td class="py-2">
                                        {{ app()->getLocale() === 'ar' ? $v->department?->name_ar ?? $v->department?->name : $v->department?->name }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="2" class="py-4 text-slate-400">
                ...
                <a href="{{ route('visits.index') }}"
                    class="mt-3 inline-block text-sm text-blue-600 hover:underline">{{ __('View All') }}</a>
            </div>
        @endcan
        ...
        @can('invoices.view')
            <div class="bg-white rounded-lg shadow p-6 border border-slate-200">
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    @if (\App\Support\RoleNav::canCreateInvoiceWithServices(auth()->user()))
                        <a href="{{ route('invoices.create') }}"
                            class="bg-orange-500  px-4 py-2 rounded text-white text-sm font-bold hover:bg-orange-600">+
                            {{ __('Add Invoice') }}</a>
                    @endif
                    <a href="{{ route('invoices.index') }}"
                        class="bg-slate-100 text-slate-600 px-4 py-2 rounded text-sm hover:bg-slate-200">{{ __('Full Page') }}</a>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-slate-600">
                        <thead>
                            <tr class="border-b">
                                <th class="text-start py-2">{{ __('Invoice Number') }}</th>
                                <th class="text-start py-2">{{ __('Total') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($recentInvoices as $inv)
                                <tr class="border-b border-slate-100 hover:bg-slate-50 cursor-pointer"
                                    onclick="window.location.href='{{ route('invoices.show', $inv) }}'">
            ...
            <div class="bg-white rounded-lg shadow p-6 border border-slate-200">
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    <a href="{{ route('charity-claims.index', ['tab' => 'insurance']) }}"
                        class="bg-blue-600  px-4 py-2 rounded text-sm font-bold hover:bg-blue-700">+
                        {{ app()->getLocale() === 'ar' ? 'مطالبة جديدة' : 'New Claim' }}</a>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-slate-600">
                        <thead>
                        ...
                        <tbody>
                            @forelse($recentClaims as $claim)
                                @php
                                    ...
                                    $url =
                                        $type === 'insurance'
                                            ? route('insurance-claims.show', $claim)
                                            : route('charity-claims.show', $claim);
                                @endphp
                                <tr class="border-b border-slate-100 hover:bg-slate-50 cursor-pointer"
                                    onclick="window.location.href='{{ $url }}'">
                                    <td class="py-2 text-blue-600 font-medium">
                                        {{ $claim->patient?->name ?? $claim->invoice?->patient?->name }}</td>
                                    <td class="py-2">
                                        <span
                                            class="px-2 py-0.5 rounded-full text-xs font-medium {{ $claim->status === 'approved' ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-800' }}">
                                            {{ $claim->status }}
                                        </span>
                                    </td>
                                </tr>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-slate-600">
                        <thead>
                            <tr class="border-b">
                                <th class="text-start py-2">{{ __('Patients') }}</th>
                                <th class="text-start py-2">{{ app()->getLocale() === 'ar' ? 'نوع الدفع' : 'Payment' }}
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($recentPatients as $p)
                                <tr class="border-b border-slate-100 hover:bg-slate-50 cursor-pointer"
                                    onclick="window.location.href='{{ route('patients.show', $p) }}'">
                            ...
                            @empty
                                <tr>
                                    <td colspan="2" class="py-4 text-slate-400">
                                        {{ app()->getLocale() === 'ar' ? 'لا يوجد مرضى' : 'No patients yet' }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <a href="{{ auth()->user()->hasRole('insurance_clerk') ? route('patients.section.insurance') : route('patients.section.followup') }}"
                    class="mt-3 inline-block text-sm text-blue-600 hover:underline">{{ __('View All') }}</a>
            </div>
        @endif
        ...
        @if (\App\Support\RoleNav::canSeeFinancialSummary(auth()->user()))
            <div class="bg-white rounded-lg shadow p-6 border border-slate-200">
                <h2 class="text-lg font-semibold text-slate-800 mb-4">
                    {{ app()->getLocale() === 'ar' ? 'ملخص مالي' : 'Financial summary' }}</h2>
                <div class="space-y-4">
                    <div class="p-4 bg-slate-50 rounded-lg">
                        <p class="flex justify-between text-slate-600 mb-1">
                            <span>{{ app()->getLocale() === 'ar' ? 'إجمالي الفواتير' : 'Total invoiced' }}</span> <span
                                class="font-bold text-slate-900">@currency($totalInvoiced)</span></p>
                        <div class="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                            <div class="bg-blue-500 h-full" style="width: 100%"></div>
                        </div>
                    </div>
                    <div class="p-4 bg-green-50 rounded-lg">
                        <p class="flex justify-between text-green-700 mb-1">
                            <span>{{ app()->getLocale() === 'ar' ? 'المحصل' : 'Collected' }}</span> <span
                                class="font-bold text-green-900">@currency($totalCollected)</span></p>
                    ...
                    <div class="p-4 bg-orange-50 rounded-lg">
                        <p class="flex justify-between text-orange-700 mb-1">
                            <span>{{ app()->getLocale() === 'ar' ? 'المتبقي' : 'Remaining' }}</span> <span
                                class="font-bold text-orange-900">@currency($totalRemaining)</span></p>
                ...
                <a href="{{ route('revenue.control-room') }}"
                    class="mt-4 inline-block text-sm text-blue-600 hover:underline font-medium">{{ __('View Detailed Reports') }}</a>
            </div>
        @endif
    </div>
    ...
    <script>
        ...
        function openDashboardModal(type, data = null) {
            ...
            if (type === 'view') {
                ...
                dynamicFields.innerHTML =
                    `<p class="text-center italic py-4 text-slate-400">${isAr ? 'جاري التحميل...' : 'Loading...'}</p>`;
                    ...
                    .then(json => {
                        if (json.success) {
                            ...
                            if (data.type === 'patient') {
                                ...
                                if (d.name_ar) html +=
                                    `<p><strong>${isAr ? 'الاسم (عربي)' : 'Name (Ar)'}:</strong> ${d.name_ar}</p>`;
                                ...
                                html +=
                                    `<p><strong>${isAr ? 'المريض' : 'Patient'}:</strong> ${d.patient?.name || '—'}</p>`;
                                html +=
                                    `<p><strong>${isAr ? 'القسم' : 'Department'}:</strong> ${isAr ? (d.department?.name_ar || d.department?.name) : d.department?.name}</p>`;
                                html +=
                                    `<p><strong>${isAr ? 'المناوبة' : 'Shift'}:</strong> ${isAr ? (d.shift?.name_ar || d.shift?.name) : d.shift?.name}</p>`;
                                html += `<p><strong>${isAr ? 'نوع الحالة' : 'Case Type'}:</strong> ${d.case_type}</p>`;
                                html +=
                                    `<p><strong>${isAr ? 'التاريخ' : 'Date'}:</strong> ${new Date(d.visit_date).toLocaleDateString()}</p>`;
                            } else {
                                html +=
                                    `<p><strong>${isAr ? 'المريض' : 'Patient'}:</strong> ${d.patient?.name || '—'}</p>`;
                                if (d.amount) html +=
                                    `<p><strong>${isAr ? 'المبلغ' : 'Amount'}:</strong> ${d.amount} SAR</p>`;
                                if (d.total_debt) html +=
                                    `<p><strong>${isAr ? 'إجمالي الدين' : 'Total Debt'}:</strong> ${d.total_debt} SAR</p>`;
                                if (d.result) html +=
                                    `<p><strong>${isAr ? 'النتيجة' : 'Result'}:</strong> ${d.result}</p>`;
                                if (d.notes) html +=
                                `<p><strong>${isAr ? 'ملاحظات' : 'Notes'}:</strong> ${d.notes}</p>`;
                                if (d.details) html +=
                                    `<p><strong>${isAr ? 'التفاصيل' : 'Details'}:</strong> ${d.details}</p>`;
                                html +=
                                    `<p><strong>${isAr ? 'التاريخ' : 'Date'}:</strong> ${d.created_at || d.inventory_date || d.contact_date || d.report_date || ''}</p>`;
                            }
                            html += '</div>';
                ...
                dynamicFields.innerHTML =
                    `<p class="py-2 text-slate-600 italic text-sm">${isAr ? 'سيتم تحويلك إلى صفحة إنشاء الفاتورة بعد اختيار المريض.' : 'You will be redirected to create invoice after selecting patient.'}</p>`;
            ...
            try {
                ...
                if (patients.length > 0) {
                    patients.forEach(p => {
                        ...
                        div.className =
                            'px-4 py-2 hover:bg-slate-50 cursor-pointer border-b border-slate-100 last:border-0';
                        const displayName = "{{ app()->getLocale() }}" === 'ar' ? (p.name_ar || p.name) : p
                            .name;
                        div.innerHTML =
                            `<span class="font-bold">${displayName}</span> <span class="text-xs text-slate-500">(${p.file_number} / ${p.identity_value})</span>`;
                    ...
                    searchResults.innerHTML =
                        `<div class="px-4 py-2 text-slate-400 italic text-sm">{{ app()->getLocale() === 'ar' ? 'لا توجد نتائج' : 'No results found' }}</div>`;
                    searchResults.classList.remove('hidden');
                }
            } catch (error) {
        ...
        async function selectPatient(p) {
            ...
            if (['commitment', 'non_commitment'].includes(type)) {
                ...
                invoiceSelect.innerHTML =
                    `<option value="">{{ app()->getLocale() === 'ar' ? 'جاري تحميل الفواتير...' : 'Loading invoices...' }}</option>`;
                try {
        ...
        async function handleDashboardSubmit(e) {
            ...
            if (!patientId && type !== 'patient_create') {
                alert(
                "{{ app()->getLocale() === 'ar' ? 'يرجى اختيار مريض أولاً' : 'Please select a patient first' }}");
                return;
            }
            ...
            if (type === 'commitment' || type === 'non_commitment') {
                ...
                if (!invoiceId) {
                    alert(
                        "{{ app()->getLocale() === 'ar' ? 'يرجى اختيار فاتورة أولاً' : 'Please select an invoice first' }}");
                    return;
                }
