@extends('layouts.app')
@section('title', app()->getLocale() === 'ar' ? 'إضافة محضر اتصال' : 'Add Contact Report')
@section('content')
    <h2 class="text-xl font-semibold text-slate-800 mb-6">{{ app()->getLocale() === 'ar' ? 'إضافة محضر اتصال' : 'Add Contact Report' }}</h2>
    <form action="{{ route('contact-reports.store') }}" method="POST" class="bg-white rounded-lg shadow p-6 max-w-xl">
        @csrf
        <div class="mb-4">
        ...
        <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 mb-1">{{ app()->getLocale() === 'ar' ? 'تاريخ الاتصال' : 'Contact date' }}</label>
            <input type="date" name="contact_date" value="{{ old('contact_date', date('Y-m-d')) }}" required class="w-full rounded border border-slate-300 px-3 py-2">
            @error('contact_date')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 mb-1">{{ app()->getLocale() === 'ar' ? 'النتيجة' : 'Result' }}</label>
            <input type="text" name="result" value="{{ old('result') }}" class="w-full rounded border border-slate-300 px-3 py-2">
        </div>
        <div class="mb-4">
        ...
        <a href="{{ route('contact-reports.index') }}" class="mr-2 text-slate-600 hover:underline">{{ app()->getLocale() === 'ar' ? 'إلغاء' : 'Cancel' }}</a>
    </form>
@endsection
