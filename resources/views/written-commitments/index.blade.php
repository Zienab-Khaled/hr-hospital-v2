@extends('layouts.app')
@section('title', app()->getLocale() === 'ar' ? 'التعهد الخطي' : 'Written Commitments')
@section('content')
...
<div x-data="dataTable()" x-init="init()">
    ...
    <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-4">
            <h2 class="text-2xl font-bold text-slate-800">{{ app()->getLocale() === 'ar' ? 'التعهد الخطي' : 'Written Commitments' }}</h2>
            <div x-show="selectedItems.length > 0" x-cloak class="flex items-center gap-3">
                ...
                @can('procedures.written_commitment')
                    <button @click="executeBulkAction('{{ route('written-commitments.bulk-delete') }}', 'DELETE', '{{ app()->getLocale() === 'ar' ? 'حذف التعهدات المحددة؟' : 'Delete selected?' }}')" class="px-4 py-2 text-sm font-bold rounded-lg bg-red-600 text-white hover:bg-red-700 shadow-md">{{ app()->getLocale() === 'ar' ? 'حذف المحدد' : 'Delete Selected' }}</button>
                @endcan
            </div>
        </div>
        @can('procedures.written_commitment')
            <a href="{{ route('written-commitments.create') }}" class="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-3 rounded-lg text-sm font-bold hover:from-blue-700 hover:to-blue-800 shadow-lg flex items-center gap-2 transition-all transform hover:scale-105">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                {{ app()->getLocale() === 'ar' ? 'إضافة تعهد خطي' : 'Add Commitment' }}
            </a>
        @endcan
    </div>

    <x-index-filters :action="route('written-commitments.index')" :searchPlaceholder="app()->getLocale() === 'ar' ? 'اسم المريض، المبلغ...' : 'Patient name, amount...'"></x-index-filters>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($items as $r)
                        <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-slate-50 transition-all duration-150" :class="{'bg-blue-50': selectedItems.includes({{ $r->id }})}">
                            ...
                            <td class="px-6 py-4 text-slate-600">{{ $r->commitment_date->format('Y-m-d') }}</td>
                            <td class="px-6 py-4 font-semibold text-slate-800">{{ $r->patient?->name }}</td>
                            <td class="px-6 py-4 font-semibold text-green-700">@currency($r->amount)</td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold 
                                    {{ $r->status === 'signed' ? 'bg-green-100 text-green-800' : ($r->status === 'pending' ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-800') }}">
                                    {{ $r->status ?? '—' }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('written-commitments.show', $r) }}" class="inline-flex items-center gap-1.5 bg-blue-600 text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-blue-700 shadow-sm transition-all">
                    ...
                    @empty
                        <tr><td colspan="7" class="px-4 py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h3 class="text-lg font-bold text-slate-700 mb-2">{{ app()->getLocale() === 'ar' ? 'لا توجد تعهدات' : 'No Commitments Found' }}</h3>
                                <p class="text-sm text-slate-500">{{ app()->getLocale() === 'ar' ? 'لم يتم العثور على تعهدات خطية' : 'No commitments found' }}</p>
                            </div>
                        </td></tr>
                    @endforelse
                </tbody>
