@php
    $showAuditActions = $showAuditActions ?? true;
    $cardPrefix = $cardPrefix ?? 'invoice';
    $cardId = $cardPrefix . '-card-' . $invoice->id;
...
@endphp
<div class="premium-card rounded-[2.5rem] p-6 transition-all hover:scale-[1.01] hover:shadow-xl group border-2 {{ $invoice->audit_status === 'rejected' ? 'border-red-100 bg-red-50/30' : 'border-transparent' }}" id="{{ $cardId }}">
    ...
    @if($invoice->rejection_reason)
        <div class="mb-6 p-4 bg-red-50 text-red-700 text-xs rounded-2xl border border-red-100 font-bold relative overflow-hidden">
    ...
    @if($showAuditActions && \App\Support\RoleNav::canAuditInControlRoom(auth()->user()))
        <div id="rejection-form-{{ $cardPrefix }}-{{ $invoice->id }}" class="hidden mb-6 p-6 bg-red-50 rounded-3xl border-2 border-red-200">
            <form action="{{ route('revenue.invoices.reject', $invoice) }}" method="POST">
                ...
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button type="button" onclick="toggleRejectionForm('{{ $cardPrefix }}-{{ $invoice->id }}')" class="text-red-400 hover:text-red-600 transition-colors text-xl font-bold">&times;</button>
                </div>
                ...
                <div class="flex gap-3">
                    <button type="submit" class="flex-1 py-3 bg-red-600 text-white text-xs font-black rounded-2xl hover:bg-red-700 transition-all shadow-lg uppercase">
                        {{ app()->getLocale() === 'ar' ? 'تأكيد الرفض' : 'Confirm Rejection' }}
                    </button>
                    <button type="button" onclick="toggleRejectionForm('{{ $cardPrefix }}-{{ $invoice->id }}')" class="px-6 py-3 bg-white text-slate-500 text-xs font-black rounded-2xl hover:bg-slate-100 transition-all border border-slate-200">
    ...
    <div class="flex items-center gap-4 pt-6 border-t border-slate-100" id="actions-{{ $cardPrefix }}-{{ $invoice->id }}">
        @if($showAuditActions && ($invoice->audit_status === 'under_review' || $invoice->audit_status === 'rejected'))
            @if(\App\Support\RoleNav::canAuditInControlRoom(auth()->user()))
                ...
                <button type="button" onclick="toggleRejectionForm('{{ $cardPrefix }}-{{ $invoice->id }}')" class="btn-reject flex-1 inline-flex items-center justify-center px-6 py-4 text-xs font-black rounded-2xl gap-3">
        ...
        @elseif($invoice->audit_status === 'matched')
            <div class="w-full py-4 bg-blue-50 text-blue-700 text-xs font-black rounded-2xl text-center border-2 border-blue-100">
                {{ app()->getLocale() === 'ar' ? 'تم التحويل لأمين الصندوق' : 'Transferred to Treasury' }}
            </div>
        @elseif($invoice->audit_status === 'ready_for_deposit')
            <div class="w-full py-4 bg-amber-50 text-amber-700 text-xs font-black rounded-2xl text-center border-2 border-amber-100">
                {{ app()->getLocale() === 'ar' ? 'جاهز للإيداع — بانتظار تأكيد المدير' : 'Ready for deposit — awaiting manager' }}
            </div>
        @elseif($invoice->audit_status === 'manager_confirmed')
            <div class="w-full py-4 bg-violet-50 text-violet-700 text-xs font-black rounded-2xl text-center border-2 border-violet-100">
                {{ app()->getLocale() === 'ar' ? 'تم التأكيد من المدير' : 'Manager confirmed' }}
            </div>
        @elseif($invoice->audit_status === 'deposited')
            <div class="w-full py-4 bg-emerald-50 text-emerald-700 text-xs font-black rounded-2xl text-center border-2 border-emerald-100">
                {{ app()->getLocale() === 'ar' ? 'تم الإيداع في البنك (إقفال)' : 'Deposited at Bank (Closed)' }}
            </div>
        @else
            <div class="w-full py-4 bg-slate-50 text-slate-600 text-xs font-black rounded-2xl text-center border-2 border-slate-100">
