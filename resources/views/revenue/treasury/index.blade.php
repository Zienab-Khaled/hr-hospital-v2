...
@section('content')
    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 bg-slate-50 min-h-screen font-cairo">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
            ...
            <form action="{{ route('revenue.treasury.index') }}" method="GET"
                class="premium-card p-5 rounded-3xl flex items-center gap-6">
                <div class="flex flex-col gap-1.5">
                    <label
                        class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">{{ app()->getLocale() === 'ar' ? 'تاريخ العمليات' : 'Operation Date' }}</label>
                ...
                <button type="submit"
                    class="inline-flex items-center px-6 py-2.5 bg-slate-900 text-white text-sm font-black rounded-2xl hover:bg-slate-800">
        ...
        <div class="mb-10 flex flex-wrap gap-4">
            <div class="premium-card rounded-2xl p-5 border-2 border-blue-200 bg-blue-50/50 flex-1 min-w-[200px]">
                <p class="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">
                    {{ app()->getLocale() === 'ar' ? 'الواردة من المحصل' : 'From Collector' }}</p>
                <p class="text-2xl font-black text-blue-800">{{ $treasuryStats['matched_count'] }}</p>
                <p class="text-sm font-bold text-blue-700 mt-1">{{ number_format($treasuryStats['matched_amount'], 2) }}
            ...
            <div class="premium-card rounded-2xl p-5 border-2 border-violet-200 bg-violet-50/50 flex-1 min-w-[200px]">
                <p class="text-[10px] font-black text-violet-600 uppercase tracking-widest mb-1">
                    {{ app()->getLocale() === 'ar' ? 'تم التأكيد من المدير' : 'Manager Confirmed' }}</p>
                <p class="text-2xl font-black text-violet-800">{{ $treasuryStats['manager_confirmed_count'] ?? 0 }}</p>
                <p class="text-sm font-bold text-violet-700 mt-1">
                    {{ number_format($treasuryStats['manager_confirmed_amount'] ?? 0, 2) }}
                    {{ app()->getLocale() === 'ar' ? 'ريال' : 'EGP' }}</p>
            </div>
        ...
        <div class="mb-12">
            <h2 class="text-xl font-black text-slate-800 flex items-center gap-3 mb-6">
                ...
                <span
                    class="px-3 py-1 rounded-full text-xs font-black bg-blue-600 text-white">{{ $matchedInvoices->count() }}</span>
            </h2>

            @forelse($matchedInvoices as $invoice)
                @php $fileLinks = $invoice->getAllRelatedMediaUrls(); @endphp
                <div
                    class="premium-card rounded-[2.5rem] p-6 mb-6 border-2 border-transparent hover:shadow-xl transition-all">
                        ...
                        <div class="flex gap-4">
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <span
                                        class="text-[10px] font-black px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">#{{ $invoice->invoice_number }}</span>
                                ...
                                <div class="flex flex-wrap gap-2 justify-end">
                                    @foreach ($fileLinks as $name => $url)
                                        <a href="{{ $url }}" target="_blank"
                                            class="px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-xl text-xs font-bold border border-indigo-100 hover:bg-indigo-100">
                                            📄 {{ Str::limit($name, 18) }}
                                        </a>
                                    @endforeach
                                </div>
            ...
            @empty
                <div class="premium-card rounded-3xl p-12 text-center border-2 border-dashed border-slate-200">
                    <p class="text-slate-400 font-bold">
                        {{ app()->getLocale() === 'ar' ? 'لا توجد فواتير واردة من المحصل بهذا التاريخ' : 'No matched invoices for this date' }}
                    </p>
                </div>
            @endforelse
        </div>
        ...
        <div class="mb-12">
            ...
            @forelse($readyForDepositInvoices as $invoice)
                @php $fileLinks = $invoice->getAllRelatedMediaUrls(); @endphp
                <div
                    class="premium-card rounded-[2.5rem] p-6 mb-6 border-2 border-amber-100 hover:shadow-xl transition-all">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div class="flex gap-4">
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <span
                                        class="text-[10px] font-black px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">#{{ $invoice->invoice_number }}</span>
                                    <a href="{{ route('invoices.show', $invoice) }}" target="_blank"
                                        class="text-[10px] font-black text-amber-700 hover:underline">
                                        {{ app()->getLocale() === 'ar' ? 'عرض الفاتورة' : 'View' }}
                                    </a>
                                </div>
                                <h3 class="text-lg font-black text-slate-900">
                        ...
                        <div class="flex flex-col items-end gap-2">
                            @if (!empty($fileLinks))
                                <div class="flex flex-wrap gap-2 justify-end">
                                    @foreach ($fileLinks as $name => $url)
                                        <a href="{{ $url }}" target="_blank"
                                            class="px-3 py-1.5 bg-amber-50 text-amber-700 rounded-xl text-xs font-bold border border-amber-200 hover:bg-amber-100">
                                            📄 {{ Str::limit($name, 18) }}
                                        </a>
                                    @endforeach
                                </div>
        ...
        <div class="mb-12">
            ...
            @forelse($managerConfirmedInvoices as $invoice)
                @php $fileLinks = $invoice->getAllRelatedMediaUrls(); @endphp
                <div
                    class="premium-card rounded-[2.5rem] p-6 mb-6 border-2 border-violet-100 hover:shadow-xl transition-all">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div class="flex gap-4">
                            <div class="w-14 h-14 rounded-2xl bg-violet-50 flex items-center justify-center text-2xl">🏦
                            </div>
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <span
                                        class="text-[10px] font-black px-2 py-0.5 rounded-full bg-violet-100 text-violet-700">#{{ $invoice->invoice_number }}</span>
                                    <a href="{{ route('invoices.show', $invoice) }}" target="_blank"
                                        class="text-[10px] font-black text-violet-700 hover:underline">
                                        {{ app()->getLocale() === 'ar' ? 'عرض الفاتورة' : 'View' }}
                                    </a>
                                </div>
                                <h3 class="text-lg font-black text-slate-900">
                        ...
                        <div class="flex flex-col items-end gap-2">
                            @if (!empty($fileLinks))
                                <div class="flex flex-wrap gap-2 justify-end">
                                    @foreach ($fileLinks as $name => $url)
                                        <a href="{{ $url }}" target="_blank"
                                            class="px-3 py-1.5 bg-violet-50 text-violet-700 rounded-xl text-xs font-bold border border-violet-200 hover:bg-violet-100">
                                            📄 {{ Str::limit($name, 18) }}
                                        </a>
                                    @endforeach
                                </div>
                            ...
                            <form action="{{ route('revenue.invoices.deposited', $invoice) }}" method="POST"
                                ...
                                <label class="flex items-center gap-2 text-xs font-bold text-slate-600 w-full justify-end">
                                    ...
                                    <input type="file" name="deposit_slip" accept="image/*"
                                        class="rounded border border-slate-300 text-[10px] max-w-[180px]">
                                </label>
                                <button type="submit"
            ...
            @empty
                <div class="premium-card rounded-3xl p-12 text-center border-2 border-dashed border-slate-200">
                    <p class="text-slate-400 font-bold">
                        {{ app()->getLocale() === 'ar' ? 'لا توجد فواتير مؤكدة من المدير بهذا التاريخ' : 'No manager-confirmed invoices for this date' }}
                    </p>
                </div>
            @endforelse
        </div>
        ...
        <div class="mt-12">
            ...
            @forelse($depositedInvoices as $invoice)
                @php $fileLinks = $invoice->getAllRelatedMediaUrls(); @endphp
                <div
                    class="premium-card rounded-[2.5rem] p-6 mb-6 border-2 border-emerald-100 hover:shadow-xl transition-all bg-emerald-50/30">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div class="flex gap-4">
                        ...
                        @if ($depositMedia->isNotEmpty())
                            <div class="flex flex-wrap gap-2 justify-end items-center">
                                <span
                                    class="text-[10px] font-bold text-emerald-700">{{ app()->getLocale() === 'ar' ? 'صورة الإيداع:' : 'Deposit slip:' }}</span>
                                @foreach ($depositMedia as $media)
                                    <a href="{{ $media->getUrl() }}" target="_blank"
                                        class="px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-xl text-xs font-bold border border-emerald-200 hover:bg-emerald-100">
