@php
    $staffPivotReady = $staffPivotReady ?? \App\Support\ShiftStaff::pivotTableReady();
...
<div class="border-t border-slate-100 pt-6">
    ...
    @else
        <div class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
            {{ app()->getLocale() === 'ar'
                ? 'ربط الموظفين بالمناوبة يتطلب تحديث قاعدة البيانات على السيرفر. اطلب من المسؤول تشغيل: php artisan migrate'
                : 'Staff assignment requires a database update on the server. Ask your admin to run: php artisan migrate' }}
        </div>
    @endif
</div>
