    ...
    <form action="{{ route('shifts.update', $shift) }}" method="POST">
        ...
        <div class="grid grid-cols-1 gap-6">
            <div>
                ...
                <input type="text" name="name_ar" value="{{ old('name_ar', $shift->name_ar) }}" placeholder="مثال: الوردية الصباحية" required
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $shift->name) }}" placeholder="e.g. Morning Shift" required
                    class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="start_time" value="{{ old('start_time', $shift->getRawOriginal('start_time') ?? $shift->start_time) }}" required
                        class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2">
                    @error('start_time') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">{{ app()->getLocale() === 'ar' ? 'وقت النهاية' : 'End Time' }}</label>
                    <input type="time" name="end_time" value="{{ old('end_time', $shift->getRawOriginal('end_time') ?? $shift->end_time) }}" required
                        class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $shift->sort_order) }}"
                        class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 px-4 py-2">
                </div>
                <div class="flex items-center pt-6">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $shift->is_active ? 'checked' : '' }}>
