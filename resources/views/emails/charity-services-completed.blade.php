            ...
            <div class="info-box" style="background: #fef3c7; border-color: #f59e0b; margin-bottom: 24px;">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value"
                        style="font-size: 18px; color: #b45309;">{{ number_format((float) $invoice->total_amount, 2) }}
                        ريال</span>
                </div>
            ...
            <div style="margin-top: 40px; display: table; width: 100%;">
                <div style="display: table-cell; text-align: right; vertical-align: bottom;">
                    ...
                    <p style="margin: 5px 0 0; font-size: 13px; color: #64748b;">
                        {{ $manager->name ?? ($settings['manager_name'] ?? '') }}</p>
                    ...
                    @if ($signaturePath && file_exists(storage_path('app/public/' . $signaturePath)))
                        <img src="{{ $message->embed(storage_path('app/public/' . $signaturePath)) }}"
                            alt="توقيع مدير إدارة تنمية الإيرادات" style="max-height: 80px; margin-top: 10px;">
                    @endif
                </div>
                ...
                @if (!empty($settings['stamp']) && file_exists(storage_path('app/public/' . $settings['stamp'])))
                    <div style="display: table-cell; text-align: left; vertical-align: bottom;">
                        <img src="{{ $message->embed(storage_path('app/public/' . $settings['stamp'])) }}"
                            alt="ختم المستشفى" style="max-height: 120px;">
                    </div>
                @endif
            </div>
        </div>
