    ...
    <div class="max-w-2xl mx-auto px-4">
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden text-center p-8">
            @if($partySend->response_action === 'confirmed')
                ...
                <h1 class="text-2xl font-bold text-green-900 mb-3">{{ app()->getLocale() === 'ar' ? 'تم تأكيد الالتزام بالدفع' : 'Payment commitment confirmed' }}</h1>
            @else
                ...
                <h1 class="text-2xl font-bold text-red-900 mb-3">{{ app()->getLocale() === 'ar' ? 'تم تسجيل الرفض' : 'Rejection recorded' }}</h1>
            @endif
            <p class="text-slate-600 mb-4">
                {{ app()->getLocale() === 'ar' ? 'رقم الفاتورة:' : 'Invoice No:' }} <strong>{{ $partySend->invoice->invoice_number }}</strong>
            </p>
