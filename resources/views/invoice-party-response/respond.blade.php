...
<head>
    ...
    <title>
        {{ $action === 'confirm' ? (app()->getLocale() === 'ar' ? 'تأكيد الالتزام بالدفع' : 'Confirm payment') : (app()->getLocale() === 'ar' ? 'رفض' : 'Reject') }}
    </title>
...
<body class="bg-gradient-to-br from-blue-50 to-slate-100 min-h-screen py-12">
    <div class="max-w-2xl mx-auto px-4">
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div
                class="p-6 {{ $action === 'confirm' ? 'bg-green-50 border-b-2 border-green-200' : 'bg-red-50 border-b-2 border-red-200' }}">
                <h1
                    class="text-2xl font-bold text-center {{ $action === 'confirm' ? 'text-green-900' : 'text-red-900' }}">
                    @if ($action === 'confirm')
            ...
            <div class="p-6">
                <div class="bg-slate-50 rounded-lg p-4 mb-6">
                    <p class="text-sm text-slate-600">
                        <strong>{{ app()->getLocale() === 'ar' ? 'المريض:' : 'Patient:' }}</strong>
                ...
                <form action="{{ route('invoice-party-response.process', $partySend->token) }}" method="POST"
                    ...
                    @endif
                    <button type="submit"
                        class="w-full py-3 rounded-lg font-bold text-lg {{ $action === 'confirm' ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-red-600 hover:bg-red-700 text-white' }}">
                        @if ($action === 'confirm')
