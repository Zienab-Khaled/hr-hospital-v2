    ...
    <div class="max-w-2xl mx-auto px-4">
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden text-center p-8">
            ...
            <h1 class="text-2xl font-bold text-slate-900 mb-3">
                {{ app()->getLocale() === 'ar' ? 'تم الرد مسبقاً' : 'Already responded' }}</h1>
            <p class="text-slate-600 mb-4">
                {{ app()->getLocale() === 'ar' ? 'تم تسجيل ردكم على هذا العرض ولا يمكن تعديله.' : 'Your response to this offer has already been recorded and cannot be changed.' }}
            </p>
            <div class="bg-slate-50 rounded-lg p-4">
                <p class="text-sm"><strong>{{ $partySend->invoice->invoice_number }}</strong> —
                    {{ $partySend->response_action === 'confirmed' ? (app()->getLocale() === 'ar' ? 'مؤكد' : 'Confirmed') : (app()->getLocale() === 'ar' ? 'مرفوض' : 'Rejected') }}
                </p>
            </div>
        </div>
