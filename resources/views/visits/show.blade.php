        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-slate-500 font-medium">
                    ...
                    {{ app()->getLocale() === 'ar' ? 'تسجيل بواسطة' : 'Registered by' }}: {{ $visit->registeredBy?->name ?? '—' }}
                </p>
            </div>
        </div>
                    ...
                    <div class="space-y-4">
                        <div class="p-4 bg-slate-50/50 rounded-2xl border border-slate-100">
                            ...
                            <div class="flex items-center gap-2">
                                <span class="w-3 h-3 rounded-full {{ $visit->case_type === 'emergency' ? 'bg-rose-500' : 'bg-emerald-500' }}"></span>
                                <p class="text-lg font-bold text-slate-700">
            ...
            <div class="glass-card rounded-3xl p-8">
                ...
                @if($visit->invoices->isEmpty())
                    <div class="text-center py-12 bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-200">
                        <div class="w-16 h-16 bg-slate-200 rounded-2xl flex items-center justify-center text-slate-400 mx-auto mb-4">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                        </div>
                        <p class="text-slate-500 font-bold">{{ app()->getLocale() === 'ar' ? 'لا توجد فواتير لهذه الزيارة' : 'No invoices for this visit' }}</p>
                    </div>
                @else
                    <div class="space-y-6">
                        @foreach($visit->invoices as $invoice)
                        <div class="border border-slate-100 rounded-3xl overflow-hidden hover:shadow-lg transition-all">
                            <div class="bg-slate-50/80 p-5 flex items-center justify-between border-b border-slate-100">
                                <div class="flex items-center gap-4">
                                    <span class="text-indigo-600 font-black">#{{ $invoice->invoice_number }}</span>
                                    <span class="status-badge {{ $invoice->status === 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }}">
                                        {{ app()->getLocale() === 'ar' ? ($invoice->status === 'paid' ? 'تم السداد' : 'بانتظار السداد') : ucfirst($invoice->status) }}
                                    </span>
                                </div>
                                <div class="text-right">
                                    <span class="text-xs text-slate-400 font-bold block">{{ app()->getLocale() === 'ar' ? 'المبلغ الإجمالي' : 'Total Amount' }}</span>
                                    <span class="text-xl font-black text-slate-900">{{ number_format($invoice->total_amount, 2) }} SAR</span>
                                </div>
                            </div>
                            ...
                            <div class="p-5">
                                <table class="w-full text-sm">
                                    <thead>
                                        <tr class="text-slate-400 text-left {{ app()->getLocale() === 'ar' ? 'text-right' : '' }}">
                                            <th class="pb-3 font-bold">{{ app()->getLocale() === 'ar' ? 'الخدمة' : 'Service' }}</th>
                                            <th class="pb-3 font-bold">{{ app()->getLocale() === 'ar' ? 'الكمية' : 'Qty' }}</th>
                                            <th class="pb-3 font-bold">{{ app()->getLocale() === 'ar' ? 'المبلغ' : 'Amount' }}</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-slate-50">
                                        ...
                                        <tr>
                                            ...
                                            <td class="py-3 font-bold text-slate-900">{{ number_format($item->total_price, 2) }}</td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                                ...
                                <div class="mt-6 pt-5 border-t border-slate-100 flex items-center justify-between">
                                    <div class="flex gap-2">
                                        <a href="{{ route('invoices.show', $invoice) }}" class="text-sm font-bold text-indigo-600 hover:text-indigo-800">
                                            {{ app()->getLocale() === 'ar' ? 'عرض الفاتورة' : 'View Invoice' }} →
                                        </a>
                                    </div>
                                    <div class="flex items-center gap-6">
                                        <div class="text-center">
                                            <span class="text-[10px] text-slate-400 font-bold block tracking-tighter">{{ app()->getLocale() === 'ar' ? 'المدفوع' : 'PAID' }}</span>
                                            <span class="text-sm font-bold text-emerald-600">{{ number_format($invoice->paid_amount, 2) }}</span>
                                        </div>
                                        <div class="text-center">
                                            <span class="text-[10px] text-slate-400 font-bold block tracking-tighter">{{ app()->getLocale() === 'ar' ? 'المتبقي' : 'BALANCE' }}</span>
                                            <span class="text-sm font-bold text-rose-500">{{ number_format($invoice->remaining_amount, 2) }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="space-y-8">
            <div class="glass-card rounded-3xl p-8 sticky top-8">
                ...
                <div class="flex flex-col items-center mb-8">
                    <div class="w-24 h-24 bg-gradient-to-br from-violet-100 to-indigo-100 rounded-full flex items-center justify-center text-indigo-600 mb-4 ring-4 ring-white shadow-md">
                        <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                    </div>
                    <h4 class="text-xl font-black text-slate-900 text-center">{{ $visit->patient?->name_ar ?? $visit->patient?->name ?? '—' }}</h4>
                    <span class="text-sm text-slate-400 font-bold mt-1">#{{ $visit->patient->file_number }}</span>
                </div>

                <div class="space-y-5">
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-slate-400 font-bold">{{ app()->getLocale() === 'ar' ? 'رقم الهوية' : 'ID Number' }}</span>
                        <span class="text-slate-700 font-black">{{ $visit->patient?->identity_value ?? '—' }}</span>
                    </div>
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-slate-400 font-bold">{{ app()->getLocale() === 'ar' ? 'نوع الدفع' : 'Payment' }}</span>
                        <span class="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-lg font-black uppercase text-[10px]">
                            {{ $visit->patient?->payment_type ?? '—' }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-slate-400 font-bold">{{ app()->getLocale() === 'ar' ? 'الهاتف' : 'Phone' }}</span>
                        <span class="text-slate-700 font-black" dir="ltr">{{ $visit->patient?->phone ?? '—' }}</span>
                    </div>

                    @if($visit->patient->insuranceCompany)
                    <div class="pt-4 border-t border-slate-50">
                        <span class="text-xs text-slate-400 font-bold block mb-2">{{ app()->getLocale() === 'ar' ? 'شركة التأمين' : 'Insurance' }}</span>
                        <p class="text-sm font-black text-slate-800">{{ $visit->patient->insuranceCompany->name_ar ?? $visit->patient->insuranceCompany->name }}</p>
                    </div>
                    @endif

                    @if($visit->patient->charityEntity)
                    <div class="pt-4 border-t border-slate-50">
                        <span class="text-xs text-slate-400 font-bold block mb-2">{{ app()->getLocale() === 'ar' ? 'الجمعية الخيرية' : 'Charity' }}</span>
                        <p class="text-sm font-black text-slate-800">{{ $visit->patient->charityEntity->name_ar ?? $visit->patient->charityEntity->name }}</p>
                    </div>
                    @endif
                </div>
