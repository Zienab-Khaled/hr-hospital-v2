...
<body>
    ...
    <div class="info-grid">
        ...
        <div class="info-item">
            <span class="info-label">{{ app()->getLocale() === 'ar' ? 'تاريخ الاستعلام' : 'Inquiry Date' }}</span>
            <div class="info-value">{{ $visit->visit_date->format('Y-m-d') }}</div>
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr class="total-row">
                    <td colspan="{{ $isInsurance ? 5 : 5 }}" style="text-align: {{ app()->getLocale() === 'ar' ? 'left' : 'right' }}; padding-{{ app()->getLocale() === 'ar' ? 'left' : 'right' }}: 15px;">
                        {{ app()->getLocale() === 'ar' ? 'الإجمالي التقديري:' : 'Estimated Total:' }}
                    </td>
