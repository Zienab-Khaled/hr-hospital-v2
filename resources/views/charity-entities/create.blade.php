@extends('layouts.app')
@section('title', app()->getLocale() === 'ar' ? 'إضافة جمعية خيرية' : 'Add Charity Entity')
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-semibold text-slate-800">{{ app()->getLocale() === 'ar' ? 'إضافة جمعية خيرية' : 'Add Charity Entity' }}</h2>
            <a href="{{ route('charity-entities.index') }}" class="text-slate-600 hover:text-slate-800 text-sm">{{ app()->getLocale() === 'ar' ? '← العودة' : '← Back' }}</a>
        </div>
        <form action="{{ route('charity-entities.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
            @csrf
