...
<div x-data="dataTable()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-4">
            <h2 class="text-2xl font-bold text-slate-800">{{ app()->getLocale() === 'ar' ? 'الجمعيات الخيرية' : 'Charity Entities' }}</h2>
            <div x-show="selectedItems.length > 0" x-cloak class="flex items-center gap-3">
                ...
                @can('charity_entities.manage')
                    <button @click="executeBulkAction('{{ route('charity-entities.bulk-delete') }}', 'DELETE', '{{ app()->getLocale() === 'ar' ? 'هل أنت متأكد من حذف الجمعيات المحددة؟' : 'Delete selected entities?' }}')" class="px-4 py-2 text-sm font-bold rounded-lg bg-red-600 text-white hover:bg-red-700 shadow-md">{{ app()->getLocale() === 'ar' ? 'حذف المحدد' : 'Delete Selected' }}</button>
                @endcan
            </div>
        </div>
        @can('charity_entities.manage')
            <a href="{{ route('charity-entities.create') }}" class="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-3 rounded-lg text-sm font-bold hover:from-blue-700 hover:to-blue-800 shadow-lg flex items-center gap-2 transition-all transform hover:scale-105">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                {{ app()->getLocale() === 'ar' ? 'إضافة جمعية خيرية' : 'Add Charity Entity' }}
            </a>
        @endcan
    </div>

    <x-index-filters :action="route('charity-entities.index')" :searchPlaceholder="app()->getLocale() === 'ar' ? 'اسم الجمعية، جهة الاتصال، الهاتف...' : 'Entity name, contact, phone...'"></x-index-filters>
    ...
    <div class="bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($entities as $e)
                        <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-slate-50 transition-all duration-150" :class="{'bg-blue-50': selectedItems.includes({{ $e->id }})}">
                            <td class="px-6 py-4"><input type="checkbox" value="{{ $e->id }}" :checked="selectedItems.includes({{ $e->id }})" @change="toggleItem({{ $e->id }})" class="w-5 h-5 rounded border-2 border-slate-300 text-blue-600 focus:ring-2 focus:ring-blue-500 cursor-pointer"></td>
                            <td class="px-6 py-4"><span class="font-mono font-bold text-slate-700 bg-slate-100 px-3 py-1 rounded text-sm">#{{ $e->id }}</span></td>
                            <td class="px-6 py-4 font-semibold text-slate-800">{{ app()->getLocale() === 'ar' ? ($e->name_ar ?: $e->name) : $e->name }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $e->contact_person ?: '-' }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $e->phone ?: '-' }}</td>
                            <td class="px-6 py-4"><span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-800">{{ $e->patients_count }}</span></td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    @can('charity_entities.manage')
                                        <a href="{{ route('charity-entities.show', $e) }}" class="inline-flex items-center gap-1.5 bg-blue-600 text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-blue-700 shadow-sm transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                                        <a href="{{ route('charity-entities.edit', $e) }}" class="inline-flex items-center gap-1.5 bg-green-600 text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-green-700 shadow-sm transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                                        <form action="{{ route('charity-entities.destroy', $e) }}" method="POST" class="inline" onsubmit="return confirm('{{ app()->getLocale() === 'ar' ? 'هل أنت متأكد؟' : 'Are you sure?' }}')">@csrf @method('DELETE')<button type="submit" class="inline-flex items-center gap-1.5 bg-red-600 text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-red-700 shadow-sm transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button></form>
                                    @endcan
                                </div>
                            </td>
                        </tr>
        ...
        @if($entities->hasPages())<div class="px-6 py-4 border-t-2 border-slate-200 bg-gradient-to-r from-slate-50 to-white">{{ $entities->links() }}</div>@endif
    </div>
</div>
