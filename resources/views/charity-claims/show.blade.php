        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-lg shadow p-5">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            <p class="text-slate-600 text-[10px] font-semibold uppercase mb-1">{{ app()->getLocale() === 'ar' ? 'إجمالي الفاتورة' : 'Invoice Total' }}</p>
                            ...
                            @foreach($allApprovals as $media)
                                <a href="{{ $media->getUrl() }}" target="_blank"
                                   class="inline-flex items-center gap-2 bg-white border border-emerald-300 text-emerald-700 px-3 py-2 rounded-lg text-sm font-semibold hover:bg-emerald-100 transition shadow-sm">
                                    📄 {{ $media->file_name }}
                                    <span class="text-[10px] text-emerald-500 bg-emerald-50 px-1 rounded border border-emerald-200 uppercase">{{ $media->extension }}</span>
                                </a>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="space-y-4">
                <div class="bg-white rounded-lg shadow p-5 sticky top-4">
                    ...
                    <form method="POST" action="{{ route('charity-claims.update-status', $charityClaim) }}" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <div>
