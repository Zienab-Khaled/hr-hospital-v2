    ...
    <div class="max-w-6xl mx-auto px-4 py-8 font-cairo">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            <div class="flex items-center gap-4">
                <div
                    class="w-16 h-16 bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-indigo-100">
            ...
            <div class="lg:col-span-4 space-y-8">
                ...
                <div class="glass-card rounded-3xl p-6 ring-1 ring-slate-100">
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center py-3 border-b border-slate-50">
                            <span
                                class="text-sm text-slate-400 font-bold">{{ app()->getLocale() === 'ar' ? 'الرقم الضريبي' : 'Identity Value' }}</span>
                            <span class="text-sm font-black text-slate-700">{{ $patient->identity_value }}</span>
                        </div>
                        <div class="flex justify-between items-center py-3 border-b border-slate-50">
                            <span
                                class="text-sm text-slate-400 font-bold">{{ app()->getLocale() === 'ar' ? 'نوع الهوية' : 'Identity Type' }}</span>
                            <span
                                class="text-sm font-black text-slate-700">{{ $patient->identity_type_label ?? $patient->identity_type }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-card rounded-3xl p-6 ring-1 ring-slate-100">
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center py-3 border-b border-slate-50">
                            <span
                                class="text-sm text-slate-400 font-bold">{{ app()->getLocale() === 'ar' ? 'تاريخ الميلاد' : 'Date of birth' }}</span>
                            <span class="text-sm font-black text-slate-700"
                                dir="ltr">{{ $patient->date_of_birth?->format('Y-m-d') ?? '—' }}</span>
                        </div>
                        <div class="flex justify-between items-center py-3 border-b border-slate-50">
                            <span
                                class="text-sm text-slate-400 font-bold">{{ app()->getLocale() === 'ar' ? 'العمر (سنوات)' : 'Age (years)' }}</span>
                            <span class="text-sm font-black text-slate-700">{{ $patient->ageInYears() ?? '—' }}</span>
                        </div>
                        <div class="flex justify-between items-center py-3 border-b border-slate-50">
                            <span
                                class="text-sm text-slate-400 font-bold">{{ app()->getLocale() === 'ar' ? 'الجنس' : 'Gender' }}</span>
                            <span class="text-sm font-black text-slate-700">
                        ...
                        <div class="flex justify-between items-center py-3 border-b border-slate-50">
                            <span
                                class="text-sm text-slate-400 font-bold">{{ app()->getLocale() === 'ar' ? 'الهاتف' : 'Phone' }}</span>
                            <span class="text-sm font-black text-slate-700"
                                dir="ltr">{{ $patient->phone ?? '—' }}</span>
                        </div>
                        <div class="flex justify-between items-center py-3 border-b border-slate-50">
                            <span
                                class="text-sm text-slate-400 font-bold">{{ app()->getLocale() === 'ar' ? 'الجنسية' : 'Nationality' }}</span>
                            <span class="text-sm font-black text-slate-700">{{ $patient->country_of_origin ?? '—' }}</span>
                        </div>
                        <div class="flex justify-between items-center py-3 border-b border-slate-50">
                            <span
                ...
                @if (in_array($patient->payment_type, ['insurance', 'charity'], true))
                    <div class="glass-card rounded-3xl p-6 ring-1 ring-slate-100 border-t-4 border-indigo-500">
                        ...
                        <div class="bg-indigo-50/50 p-4 rounded-2xl flex items-center gap-4">
                            <div
                                class="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center text-indigo-600">
                    ...
                    <div class="glass-card rounded-3xl p-6 ring-1 ring-slate-100 border-t-4 border-emerald-500">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($completedDepartments as $dept)
                                <span
                                    class="inline-flex items-center px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-800 border border-emerald-200 text-sm font-bold">
                ...
                @if ($patient->notes)
                    <div class="glass-card rounded-3xl p-6 ring-1 ring-slate-100">
                        <span
                            class="text-xs text-slate-400 font-black uppercase tracking-widest block mb-2">{{ app()->getLocale() === 'ar' ? 'ملاحظات خلفية' : 'Background Notes' }}</span>
            ...
            <div class="lg:col-span-8 space-y-8 mt-1.5">
                ...
                <div class="glass-card rounded-3xl overflow-hidden ring-1 ring-slate-100">
                    <div class="p-6 border-b border-slate-50 flex items-center justify-between">
                        ...
                        <span
                            class="bg-indigo-50 text-indigo-600 px-4 py-1.5 rounded-xl font-black text-sm">{{ $visits->total() }}</span>
                    </div>

                    @if ($visits->isEmpty())
                        <div class="p-12 text-center">
                            <p class="text-slate-400 font-bold">
                                {{ app()->getLocale() === 'ar' ? 'لا توجد زيارات مسجلة' : 'No visits recorded yet' }}</p>
                        </div>
                    @else
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="bg-slate-50/50 text-slate-400">
                                        <th
                                            class="px-6 py-4 text-left {{ app()->getLocale() === 'ar' ? 'text-right' : '' }} font-black">
                                            {{ app()->getLocale() === 'ar' ? 'التاريخ' : 'Date' }}</th>
                                        <th
                                            class="px-6 py-4 text-left {{ app()->getLocale() === 'ar' ? 'text-right' : '' }} font-black">
                                            {{ app()->getLocale() === 'ar' ? 'القسم الطبي' : 'Medical Dept' }}</th>
                                        <th
                                            class="px-6 py-4 text-left {{ app()->getLocale() === 'ar' ? 'text-right' : '' }} font-black">
                                            {{ app()->getLocale() === 'ar' ? 'مسار الدخول' : 'Admission' }}</th>
                                        <th
                                            class="px-6 py-4 text-left {{ app()->getLocale() === 'ar' ? 'text-right' : '' }} font-black">
                                            {{ app()->getLocale() === 'ar' ? 'نوع الحالة' : 'Case' }}</th>
                                        <th class="px-6 py-4 text-center font-black">
                                            {{ app()->getLocale() === 'ar' ? 'الإجراء' : 'Action' }}</th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr class="hover-row transition-colors">
                                            ...
                                            <td class="px-6 py-4">
                                                @if ($visit->admission_entry_source)
                                                    <span
                                                        class="inline-flex px-2 py-0.5 rounded-md text-[11px] font-black {{ $visit->admissionEntrySourceBadgeClass() }}">{{ $visit->admission_entry_source_label }}</span>
                                                @else
                                            ...
                                            <td class="px-6 py-4">
                                                @if ($visit->case_type === 'emergency')
                                                    <span
                                                        class="text-rose-600 font-black">{{ app()->getLocale() === 'ar' ? '🚨 طوارئ' : '🚨 Emergency' }}</span>
                                                @else
                                                    <span
                                                        class="text-emerald-600 font-bold">{{ app()->getLocale() === 'ar' ? '🏥 عيادات' : '🏥 Clinics' }}</span>
                                                @endif
                                            </td>
                ...
                <div class="glass-card rounded-3xl overflow-hidden ring-1 ring-slate-100">
                    <div class="p-6 border-b border-slate-50 flex items-center justify-between">
                        ...
                        <span
                            class="bg-blue-50 text-blue-600 px-4 py-1.5 rounded-xl font-black text-sm">{{ $invoices->total() }}</span>
                    </div>

                    @if ($invoices->isEmpty())
                        <div class="p-12 text-center">
                            <p class="text-slate-400 font-bold">
                                {{ app()->getLocale() === 'ar' ? 'لا توجد فواتير مسجلة' : 'No invoices recorded yet' }}</p>
                        </div>
                    @else
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="bg-slate-50/50 text-slate-400">
                                        <th
                                            class="px-6 py-4 text-left {{ app()->getLocale() === 'ar' ? 'text-right' : '' }} font-black">
                                            {{ app()->getLocale() === 'ar' ? 'رقم الفاتورة' : 'Invoice #' }}</th>
                                        <th
                                            class="px-6 py-4 text-left {{ app()->getLocale() === 'ar' ? 'text-right' : '' }} font-black">
                                            {{ app()->getLocale() === 'ar' ? 'التاريخ' : 'Date' }}</th>
                                        <th
                                        ...
                                        <th class="px-6 py-4 text-center font-black">
                                            {{ app()->getLocale() === 'ar' ? 'الحالة' : 'Status' }}</th>
                                        <th class="px-6 py-4 text-center font-black">
                                            {{ app()->getLocale() === 'ar' ? 'الإجراء' : 'Action' }}</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-50">
                                    @foreach ($invoices as $invoice)
                                        <tr class="hover-row transition-colors">
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                <span
                                                    class="status-badge {{ $invoice->status === 'paid' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600' }}">
                                                    {{ app()->getLocale() === 'ar' ? ($invoice->status === 'paid' ? 'تم السداد' : 'بانتظار') : ucfirst($invoice->status) }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 text-center">
                                                <a href="{{ route('invoices.show', $invoice) }}"
                                                    class="inline-flex items-center gap-1.5 px-4 py-1.5 bg-blue-50 text-blue-600 rounded-lg font-black text-xs hover:bg-blue-600 hover:text-white transition-all">
                                                    <span>{{ app()->getLocale() === 'ar' ? 'عرض' : 'View' }}</span>
                                                    <svg class="w-3 h-3" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
