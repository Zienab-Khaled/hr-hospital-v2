...
<div x-data="dataTable()" x-init="init()">
    ...
    <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-4">
            <h2 class="text-2xl font-bold text-slate-800">{{ app()->getLocale() === 'ar' ? 'محضر عدم تعهد' : 'Non-Commitment Reports' }}</h2>
            <div x-show="selectedItems.length > 0" x-cloak class="flex items-center gap-3">
                ...
                @can('procedures.non_commitment')
                    <button @click="executeBulkAction('{{ route('non-commitment-reports.bulk-delete') }}', 'DELETE', '{{ app()->getLocale() === 'ar' ? 'حذف المحاضر المحددة؟' : 'Delete selected?' }}')" class="px-4 py-2 text-sm font-bold rounded-lg bg-red-600 text-white hover:bg-red-700 shadow-md">{{ app()->getLocale() === 'ar' ? 'حذف المحدد' : 'Delete Selected' }}</button>
                @endcan
            </div>
        </div>
        @can('procedures.non_commitment')
            <a href="{{ route('non-commitment-reports.create') }}" class="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-3 rounded-lg text-sm font-bold hover:from-blue-700 hover:to-blue-800 shadow-lg flex items-center gap-2 transition-all transform hover:scale-105">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                {{ app()->getLocale() === 'ar' ? 'إضافة محضر' : 'Add Report' }}
            </a>
        @endcan
    </div>

    <x-index-filters :action="route('non-commitment-reports.index')" :searchPlaceholder="app()->getLocale() === 'ar' ? 'اسم المريض، التاريخ...' : 'Patient name, date...'"></x-index-filters>
            ...
            <table class="w-full">
                <thead class="bg-gradient-to-r from-slate-700 to-slate-800 text-white">
                    <tr>
                        ...
                        <th class="text-start px-6 py-4 font-bold uppercase tracking-wider text-sm">{{ app()->getLocale() === 'ar' ? 'ملاحظات' : 'Notes' }}</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($reports as $r)
                        <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-slate-50 transition-all duration-150" :class="{'bg-blue-50': selectedItems.includes({{ $r->id }})}">
                            ...
                            <td class="px-6 py-4 text-slate-600">{{ $r->report_date->format('Y-m-d') }}</td>
                            <td class="px-6 py-4 font-semibold text-slate-800">{{ $r->patient?->name }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ Str::limit($r->notes, 60) }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('non-commitment-reports.show', $r) }}" class="inline-flex items-center gap-1.5 bg-blue-600 text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-blue-700 shadow-sm transition-all">
                    ...
                    @empty
                        <tr><td colspan="6" class="px-4 py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm text-slate-500">{{ app()->getLocale() === 'ar' ? 'لم يتم العثور على محاضر عدم تعهد' : 'No non-commitment reports found' }}</p>
                            </div>
                        </td></tr>
                    @endforelse
                </tbody>
