...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold text-slate-800">{{ app()->getLocale() === 'ar' ? 'حصر ديون' : 'Debt Inventories' }}
        </h2>
        @can('procedures.debt_inventory')
            <a href="{{ route('debt-inventories.create') }}"
                class="bg-blue-600 text-white px-4 py-2 rounded text-sm hover:bg-blue-700">+
                {{ app()->getLocale() === 'ar' ? 'إضافة حصر ديون' : 'Add' }}</a>
        @endcan
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-50 border-b">
                <tr>
                    ...
                    <th class="text-start p-3">{{ app()->getLocale() === 'ar' ? 'إجمالي الدين' : 'Total debt' }}</th>
                </tr>
            </thead>
            <tbody>
                @forelse($items as $r)
                    <tr class="border-b hover:bg-slate-50">
                        <td class="p-3">{{ $r->inventory_date->format('Y-m-d') }}</td>
                        <td class="p-3">{{ $r->patient?->name }}</td>
                        <td class="p-3">@currency($r->total_debt)</td>
                    </tr>
                @empty
                    <tr>
