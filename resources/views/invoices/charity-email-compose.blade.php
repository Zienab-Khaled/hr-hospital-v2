        ...
        <div class="bg-white rounded-lg shadow-lg border-2 border-slate-200 overflow-hidden">
            <div class="p-6 border-b border-slate-200 bg-slate-50">
                ...
                <p class="text-slate-600 mt-1">
                    {{ app()->getLocale() === 'ar' ? 'الفاتورة' : 'Invoice' }}: <strong>{{ $invoice->invoice_number }}</strong>
                    — {{ $invoice->patient?->charityEntity?->name_ar ?? $invoice->patient?->charityEntity?->name ?? '—' }}
                </p>
            </div>

            <form action="{{ route('invoices.send-charity-price-offer', $invoice) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf

                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">{{ app()->getLocale() === 'ar' ? 'موضوع الإيميل' : 'Email subject' }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">{{ app()->getLocale() === 'ar' ? 'نص الرسالة (اختياري — إن تركت فارغاً يُستخدم النص الافتراضي)' : 'Message body (optional — leave blank for default)' }}</label>
                        <textarea name="custom_intro" rows="5" class="w-full rounded border border-slate-300 px-3 py-2 @error('custom_intro') border-red-500 @enderror"
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">{{ app()->getLocale() === 'ar' ? 'مرفقات إضافية (مثل التقرير الطبي)' : 'Additional attachments (e.g. medical report)' }}</label>
                        ...
                        <p class="text-xs text-slate-500 mt-1">{{ app()->getLocale() === 'ar' ? 'سيُرفق عرض السعر تلقائياً كـ PDF. يمكنك إرفاق التقرير الطبي أو غيره (PDF، Word، صور).' : 'Price offer PDF is attached automatically. You can add medical report or other files (PDF, Word, images).' }}</p>
                        @error('attachments.*')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
