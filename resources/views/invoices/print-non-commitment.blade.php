...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sig-preview { display: none; max-width: 250px; max-height: 80px; }
        .btn-gray { background: #e5e7eb; color: #333; border: none; border-radius: 6px; padding: 6px 14px; cursor: pointer; font-family: inherit; }
...
<body>
    ...
    <div class="pledge-box">
        ...
        <div class="pledge-text">
            أفيدكم بأنني امتنعت عن التوقيع على محضر التعهد الخطي بسداد كافة المصاريف العلاجية الإضافية التي تكون خارج نطاق التغطية التأمينية ، وقد أُشعرت بالتبعات المترتبة على ذلك وعلى هذا يتم التوقيع .
        </div>
    ...
    <table class="ack-table">
        <thead>
            <tr><th colspan="2">المُقَرّ بما فيه</th></tr>
        </thead>
        <tbody>
            <tr>
                <td class="row-label">الأسم :</td>
                <td>
                    <span class="fill-dots">@if($invoice->patient){{ $invoice->patient->name_ar ?? $invoice->patient->name }}@endif</span>
                </td>
            </tr>
            <tr>
                <td class="row-label">الجنسية :</td>
                <td>
                    <span class="fill-dots">@if($invoice->patient){{ $invoice->patient->country_of_origin ?? '' }}@endif</span>
                </td>
            </tr>
            <tr>
                <td class="row-label">رقم الإقامة :</td>
                <td>
                    <span class="fill-dots">@if($invoice->patient){{ $invoice->patient->identity_value ?? '' }}@endif</span>
                </td>
            </tr>
            <tr>
                <td class="row-label">التاريخ :</td>
                <td>
                    <span class="fill-dots">{{ $invoice->invoice_date?->format('Y/m/d') ?? now()->format('Y/m/d') }}</span>
                </td>
            </tr>
            <tr>
