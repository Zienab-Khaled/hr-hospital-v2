...
<head>
    ...
    <title>{{ app()->getLocale() === 'ar' ? 'محضر تعهد' : 'Commitment Form' }} - {{ $invoice->invoice_number }}</title>
    <style>
...
<body>
    ...
    <h1>{{ app()->getLocale() === 'ar' ? 'محضر تعهد' : 'Commitment Form' }}</h1>
    ...
    @if($invoice->patient)
        <div class="section">
            <div class="section-title">{{ app()->getLocale() === 'ar' ? 'بيانات المريض / الملتزم' : 'Patient / Committer' }}</div>
            <table>
    ...
    <div class="section">
        <div class="section-title">{{ app()->getLocale() === 'ar' ? 'المبلغ المعني بالتعهد' : 'Committed Amount' }}</div>
        <p><strong>@currency($invoice->total_amount)</strong> {{ app()->getLocale() === 'ar' ? '(إجمالي الفاتورة)' : '(Invoice total)' }}</p>
    </div>
