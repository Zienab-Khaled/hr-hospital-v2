            ...
            <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('invoices.print', $invoice) }}" target="_blank" rel="noopener"
                        class="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg font-semibold text-sm hover:bg-blue-700 shadow-md">
            ...
            <form action="{{ route('invoices.update', $invoice) }}" method="POST" id="invoice-form" class="space-y-6">
                ...
                <div class="bg-gradient-to-r from-blue-50 to-blue-100 border-2 border-blue-300 rounded-lg p-5 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
                        <div>
                            <span
                                class="block text-blue-700 font-semibold mb-1">{{ app()->getLocale() === 'ar' ? 'الاسم:' : 'Name:' }}</span>
                            <span class="font-medium text-slate-800">{{ $invoice->patient?->name ?? '—' }}</span>
                        </div>
                        <div>
                            <span
                                class="block text-blue-700 font-semibold mb-1">{{ app()->getLocale() === 'ar' ? 'رقم الملف:' : 'File No:' }}</span>
                            <span class="font-medium text-slate-800">{{ $invoice->patient?->file_number ?? '—' }}</span>
                        </div>
                        <div>
                            <span
                                class="block text-blue-700 font-semibold mb-1">{{ app()->getLocale() === 'ar' ? 'نوع الدفع:' : 'Payment Type:' }}</span>
                            <span
                                class="font-medium text-slate-800">{{ $invoice->patient?->payment_type_label ?? '—' }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <select name="status" required class="{{ $inputClass }}">
                            ...
                            @foreach ($statuses as $val => $labels)
                                <option value="{{ $val }}"
                                    {{ old('status', $invoice->status) === $val ? 'selected' : '' }}>
                        ...
                        <table class="w-full border-collapse" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
                            ...
                            <tbody id="services-container">
                                ...
                                @foreach ($invoice->items as $i => $item)
                                    <tr class="service-row border-b border-slate-400 hover:bg-slate-50"
                                        data-index="{{ $i }}">
                                        <td
                                            class="border border-slate-400 px-2 py-2 text-center text-sm font-medium text-slate-800">
                                            {{ $item->service?->code ?? '—' }}
                                        </td>
                                        <td class="border border-slate-400 px-2 py-2">
                                            <input type="hidden" name="services[{{ $i }}][service_id]"
                                        ...
                                        <td class="border border-slate-400 px-2 py-2">
                                            <input type="number" name="services[{{ $i }}][quantity]"
                                                value="{{ old("services.$i.quantity", $item->quantity) }}" min="1"
                                                step="1" required onchange="calculateRowTotal(this)"
            ...
            @if (\App\Support\RoleNav::canExecuteServices(auth()->user()))
                <div class="mt-8 border-2 border-emerald-300 rounded-lg p-6 bg-gradient-to-br from-emerald-50 to-slate-50">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        @endphp
                        <span
                            class="text-sm font-semibold px-3 py-1 rounded-full {{ $completedCount === $totalCount && $totalCount > 0 ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' : 'bg-amber-100 text-amber-800 border border-amber-300' }}">
                            {{ $completedCount }}/{{ $totalCount }}
                            {{ app()->getLocale() === 'ar' ? 'منفذة' : 'executed' }}
                        </span>
                    </div>
                        ...
                        <table class="w-full border-collapse" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
                            <thead>
                                <tr class="bg-slate-100 border-b-2 border-slate-300">
                                    <th class="border border-slate-300 px-3 py-2 text-center text-sm font-bold text-slate-700">
                                        {{ app()->getLocale() === 'ar' ? 'الرمز' : 'Code' }}</th>
                                    <th class="border border-slate-300 px-3 py-2 text-center text-sm font-bold text-slate-700">
                            ...
                            <tbody>
                                @foreach ($invoice->items as $item)
                                    <tr
                                        class="border-b border-slate-200 {{ $item->isCompleted() ? 'bg-emerald-50/50' : '' }}">
                                        <td class="border border-slate-200 px-2 py-2 text-center text-sm text-slate-600">
                ...
                <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
                    ...
                    <form id="execute-modal-form" method="POST">
                        ...
                        <div class="flex gap-3">
                            <button type="submit"
                                class="flex-1 bg-blue-600 text-white px-4 py-2.5 rounded-lg font-bold hover:bg-blue-700 shadow">
            ...
            function doSearch() {
                ...
                fetch(searchUrl + '?q=' + encodeURIComponent(q))
                    ...
                    .then(list => {
                        ...
                        if (!list.length) {
                            resultsDiv.innerHTML = '<p class="p-4 text-slate-700 font-medium">' + (isArabic ?
                            ...
                            list.forEach(s => {
                                ...
                                el.textContent = name + (s.code ? ' (' + s.code + ')' : '') + ' — ' + s
                                    .default_price;
                    ...
                    .catch(() => {
                        resultsDiv.innerHTML = '<p class="p-4 text-red-700">' + (isArabic ? 'خطأ في البحث' :
                            'Search error') + '</p>';
                        resultsDiv.classList.remove('hidden');
                    });
            }
