...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold text-slate-800">
            {{ app()->getLocale() === 'ar' ? 'الموظفين والصلاحيات' : 'Employees & Permissions' }}</h2>
        @can('users.manage')
            <a href="{{ route('users.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded text-sm hover:bg-blue-700">+
                {{ app()->getLocale() === 'ar' ? 'إضافة موظف' : 'Add Employee' }}</a>
        @endcan
    </div>

    {{-- Search and Filter using Global Component --}}
    <x-index-filters :action="route('users.index')" :searchPlaceholder="app()->getLocale() === 'ar' ? 'اسم المستخدم، البريد، الاسم...' : 'Username, email, name...'">
        <div class="w-40">
                    ...
                    <tr class="border-b border-slate-100 hover:bg-slate-50">
                        ...
                        <td class="p-3">
                            <div class="flex gap-2">
                                @can('users.manage')
                                    <a href="{{ route('users.show', $u) }}" class="text-blue-600 hover:text-blue-800"
                                    ...
                                    <a href="{{ route('users.edit', $u) }}" class="text-green-600 hover:text-green-800"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="p-6 text-center text-slate-500">
                            {{ app()->getLocale() === 'ar' ? 'لا يوجد مستخدمون' : 'No users' }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
