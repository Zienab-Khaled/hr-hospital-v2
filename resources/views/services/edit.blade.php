...
@section('title', app()->getLocale() === 'ar' ? 'تعديل خدمة' : 'Edit Service')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold text-slate-800">{{ app()->getLocale() === 'ar' ? 'تعديل خدمة' : 'Edit Service' }}</h2>
    ...
    <form action="{{ route('services.update', $service) }}" method="POST" class="bg-white rounded-lg shadow p-6">
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $service->name) }}" required class="w-full rounded border border-slate-300 px-3 py-2 @error('name') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="name_ar" value="{{ old('name_ar', $service->name_ar) }}" class="w-full rounded border border-slate-300 px-3 py-2 @error('name_ar') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $service->code) }}" required class="w-full rounded border border-slate-300 px-3 py-2 @error('code') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="number" name="default_price" value="{{ old('default_price', $service->default_price) }}" step="0.01" min="0" required class="w-full rounded border border-slate-300 px-3 py-2 @error('default_price') border-red-500 @enderror">
            ...
            <div>
                <div class="flex items-center mb-2">
                    ...
                    <input type="checkbox" name="is_multi_session" id="is_multi_session" value="1" {{ old('is_multi_session', $service->is_multi_session) ? 'checked' : '' }} class="rounded border-slate-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500" onclick="toggleMultiSession()">
            ...
            <div id="multi_session_fields" class="{{ old('is_multi_session', $service->is_multi_session) ? '' : 'hidden' }} grid grid-cols-1 md:grid-cols-3 gap-4 border-t pt-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">{{ app()->getLocale() === 'ar' ? 'عدد الجلسات' : 'Number of Sessions' }}</label>
                    <input type="number" name="session_count" value="{{ old('session_count', $service->session_count) }}" min="1" class="w-full rounded border border-slate-300 px-3 py-2">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">{{ app()->getLocale() === 'ar' ? 'المدة بين كل جلسة' : 'Wait Time' }}</label>
                    <input type="number" name="session_wait_time" value="{{ old('session_wait_time', $service->session_wait_time) }}" min="1" class="w-full rounded border border-slate-300 px-3 py-2">
                </div>
                <div>
                    ...
                    <select name="session_wait_unit" class="w-full rounded border border-slate-300 px-3 py-2">
                        <option value="days" {{ old('session_wait_unit', $service->session_wait_unit) == 'days' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'يوم' : 'Days' }}</option>
                        <option value="weeks" {{ old('session_wait_unit', $service->session_wait_unit) == 'weeks' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'أسبوع' : 'Weeks' }}</option>
                        <option value="months" {{ old('session_wait_unit', $service->session_wait_unit) == 'months' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'شهر' : 'Months' }}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $service->is_active) ? 'checked' : '' }} class="rounded border-slate-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
