            ...
            <tbody>
                @forelse($services as $s)
                    <tr class="border-b border-slate-100 hover:bg-slate-50">
                        <td class="p-3">
                            <div class="font-medium text-slate-800">{{ app()->getLocale() === 'ar' ? ($s->name_ar ?: $s->name) : $s->name }}</div>
                            @if(!$s->is_active)
                                <span class="text-xs text-red-500">{{ app()->getLocale() === 'ar' ? 'غير نشط' : 'Inactive' }}</span>
                            @endif
                        </td>
                        <td class="p-3">{{ $s->code }}</td>
                        <td class="p-3">{{ number_format($s->default_price, 2) }}</td>
                        <td class="p-3">{{ app()->getLocale() === 'ar' ? ($s->department?->name_ar ?: $s->department?->name) : $s->department?->name ?? '-' }}</td>
                        @can('services.manage')
                            <td class="p-3">
                                <div class="flex gap-2">
                                    <a href="{{ route('services.show', $s) }}" class="text-blue-600 hover:text-blue-800" title="{{ app()->getLocale() === 'ar' ? 'عرض' : 'View' }}">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    <a href="{{ route('services.edit', $s) }}" class="text-green-600 hover:text-green-800" title="{{ app()->getLocale() === 'ar' ? 'تعديل' : 'Edit' }}">
                ...
                @empty
                    <tr><td colspan="{{ auth()->user()->can('services.manage') ? '5' : '4' }}" class="p-6 text-center text-slate-500">{{ app()->getLocale() === 'ar' ? 'لا توجد خدمات' : 'No services yet' }}</td></tr>
                @endforelse
            </tbody>
        </table>
