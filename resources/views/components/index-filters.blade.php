...
<div class="bg-white rounded-lg shadow-sm border border-slate-200 p-3 mb-4">
    <form method="GET" action="{{ $action ?? url()->current() }}" class="flex flex-wrap gap-2 items-end">
        ...
        <div class="flex gap-2">
            ...
            @if(request()->hasAny(['search', 'per_page', 'identity_type', 'charity_entity_id', 'gender', 'age_from', 'age_to', 'insurance_company_id', 'payment_type', 'date_from', 'date_to', 'country_of_origin', 'status', 'date', 'shift_id', 'department_id', 'category', 'registered_by', 'admission_entry_source']) || count(request()->except(['page'])) > 0)
