...
@endif -->
        ...
        @if (RoleNav::canSeeControlRoom($user))
            <a href="{{ route('revenue.control-room') }}"
                ...
                <span>
                    @if (RoleNav::hasSupervisorVisibility($user))
                        {{ app()->getLocale() === 'ar' ? 'دورة الإيراد (غرفة التحكم)' : 'Revenue Cycle (Control Room)' }}
                    @else
                        {{ app()->getLocale() === 'ar' ? 'غرفة التحكم (المحاسب)' : 'Control Room (Accountant)' }}
                    @endif
                </span>
            </a>
        @endif
