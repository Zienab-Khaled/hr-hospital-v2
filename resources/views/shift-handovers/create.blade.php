@extends('layouts.app')
@section('title', app()->getLocale() === 'ar' ? 'تسليم المناوبة' : 'Hand Over Shift')
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <form action="{{ route('shift-handovers.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">{{ app()->getLocale() === 'ar' ? 'المناوبة المُسلّمة' : 'Shift being handed over' }}</label>
                        ...
                        @if ($currentShift)
                            <p class="text-xs text-slate-500 mt-1">{{ app()->getLocale() === 'ar' ? 'الافتراضي: المناوبة الحالية' : 'Default: current shift' }}</p>
                        @endif
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">{{ app()->getLocale() === 'ar' ? 'ملاحظات للمناوبة القادمة (اختياري)' : 'Notes for next shift (optional)' }}</label>
